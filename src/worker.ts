...
import { blockProducer, namespace } from "./config";
...
import { getBlob, saveBlob } from "./celestia/celestia";
...
export class DomainNameServiceWorker extends zkCloudWorker {
  ...
  private async compile(
    params: {
      compileSmartContracts?: boolean;
      compileMapUpdate?: boolean;
      compileValidators?: boolean;
    } = {}
  ): Promise<void> {
    const compileSmartContracts = params.compileSmartContracts ?? false;
    const compileValidators =
      params.compileValidators ?? compileSmartContracts ?? false;
    const compileMapUpdate =
      params.compileMapUpdate ?? compileSmartContracts ?? false;
    try {
  ...
  public async create(transaction: string): Promise<string | undefined> {
    try {
      ...
      if (proofsOff === false) {
        ...
        await this.compile({ compileMapUpdate: true });
  ...
  private async firstBlock(): Promise<string | undefined> {
    const height = await saveBlob({
      namespace: namespace.block,
      fields: BlockData.toFields(firstBlockData),
    });
    if (height === undefined) {
      console.error("First block height is undefined");
      return "First block height is undefined";
    }
    await this.cloud.saveDataByKey("blockHeight", height.toString());
  ...
  */
  ...
  private async proveRollupBlock(): Promise<string | undefined> {
    ...
    return txSent.hash;
    */
    return "proveBlock is not implemented";
  }
  ...
  private async createRollupBlock(
    ...
    */
    const previousBlockAddress = await this.cloud.getDataByKey("blockHeight");
    if (previousBlockAddress === undefined) {
      console.error("createRollupBlock: previousBlockAddress is undefined");
      return "previousBlockAddress is undefined";
    }
    const previousBlockHeight = Number(previousBlockAddress);

    let previousValidBlockAddress = previousBlockAddress;
    console.log("previousBlockAddress", previousBlockAddress);

    if (previousBlockAddressVar !== undefined) {
      const { address, timeStarted } = JSON.parse(previousBlockAddressVar);
      if (address !== undefined && timeStarted !== undefined) {
        if (
          address !== previousBlockAddress &&
    ...
    */
    const previousBlockData = await getBlob({
      namespace: namespace.block,
      height: Number(previousBlockAddress),
    });
    if (previousBlockData === undefined) {
      console.error("createRollupBlock: previousBlockData is undefined");
      return "previousBlockData is undefined";
    }
    const previousBlock = BlockData.fromFields(previousBlockData);
    const blockNumber = Number(previousBlock.blockNumber.toBigInt()) + 1;
    let previousValidBlockParams = BlockParams.unpack(
      previousBlock.blockParams
    );
    /*
    if (
      previousValidBlockParams.isFinal.toBoolean() === false &&
    ...
    */
    ...
    await this.cloud.saveDataByKey(
      "lastBlockAddress",
      JSON.stringify(
        { address: makeString(20), timeStarted: Date.now() },
        null,
        2
      )
    );
    ...
    */
    ...
    console.log(`Creating block ${blockNumber}`, blockNames);

    let database: DomainDatabase = new DomainDatabase();
    let map = new MerkleMap();
    const previousBlockRoot = previousBlock.root;
    if (blockNumber > 1) {
      const storage = previousBlock.storage;
    ...
    /*
    await this.compile({ compileValidators: true });
    ...
    const decision = new ValidatorsDecision({
      ...
      data: BlockCreationData.toFields({
        ...
        previousBlockHeight: UInt64.from(previousBlockHeight),
        notUsed: [Field(0), Field(0)],
      }),
      expiry: UInt64.from(Date.now() + 1000 * 60 * 60 * 24 * 2000),
    });
    ...
    */
    ...
    try {
      const newBlockHeight = await saveBlob({
        namespace: namespace.block,
        fields: BlockData.toFields(blockData),
      });
      if (newBlockHeight === undefined) {
        console.error("newBlockHeight is undefined");
        return "newBlockHeight is undefined";
      }
      console.log("Block saved to Celestia:", newBlockHeight);
      await this.cloud.saveDataByKey("blockHeight", newBlockHeight.toString());
      await this.cloud.saveDataByKey(
        "lastBlockAddress",
        JSON.stringify(
          { address: newBlockHeight.toString(), timeStarted: Date.now() },
          null,
          2
        )
      );
...
*/
      ...
      return newBlockHeight.toString();
    } catch (error) {
